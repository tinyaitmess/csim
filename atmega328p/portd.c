...
/* PIND register functions */

static void name_PIND(csim_inst_t *inst, int num, char *__buffer, int size) {
	portd_inst_t *__inst = (portd_inst_t *)inst;
	snprintf(__buffer, size, "portd%d_PIND", inst->number);
}


static void display_PIND(csim_inst_t *inst, int num, char *__buffer, int size) {
	portd_inst_t *__inst = (portd_inst_t *)inst;
	snprintf(__buffer, size, "%d", __inst->PIND);;
}

static csim_word_t read_PIND(csim_inst_t *inst, int num) {
		portd_inst_t *__inst = (portd_inst_t *)inst;
				return __inst->PIND;
}
 
static csim_word_t get_PIND(csim_inst_t *inst, int num) {
		portd_inst_t *__inst = (portd_inst_t *)inst;
			return __inst->PIND;
}
 
static void set_PIND(csim_inst_t *inst, int num, csim_word_t val) {
	portd_inst_t *__inst = (portd_inst_t *)inst;
		if(__inst->PIND != val) {
			__inst->PIND = val;
			on_update_all(inst);
...
static void write_PIND(csim_inst_t *inst, int num, csim_word_t val) {
	portd_inst_t *__inst = (portd_inst_t *)inst;
	set_PIND(inst, num, val);
...
static void on_input_PIN(csim_inst_t *inst, csim_value_type_t type, csim_value_t val);
...
static void on_input_PIN(csim_inst_t *inst, csim_value_type_t type, csim_value_t val) {
	...
	int ____INDEX = __inst->inst.comp.ports - (ports + PIN_BASE);
