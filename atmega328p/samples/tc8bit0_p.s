// Test du composant T/C 8 bits 0 - Mode PWM
...
_start:
    ...
    // Configuration du mode PWM (WGM = 011 => Mode Fast PWM)
    mov     R8, #0x03               // WGM bits (011 pour Fast PWM)
    strb    R8, [R3]                // Ecriture dans TCCR0A

    // Configurer le prescaler (CS02:0 = 011 -> prescaler 64)
    mov     R8, #0x03               // CS02:0 = 011
    strb    R8, [R4]                // Ecriture dans TCCR0B

    // Configuration des valeurs OCR0A et OCR0B pour tester PWM
...
_loop:
    ...
    tst     R9, #0x02                     // Tester le bit OCF0A (signal de comparaison)
    bne     led_on                        // Si OCF0A == 1, allumer LED

    tst     R9, #0x04                     // Tester le bit OCF0B (signal de comparaison)
