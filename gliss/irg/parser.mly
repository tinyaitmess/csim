...
top:
	csim_top specs EOF		{ Sem.final_checks (Iter.get_insts()) }
;

csim_top:
	/* empty */
		{ Csim.prepare_parsing () }
;
...
LetDef	:
	LET csim_other OptStar LocatedID EQ LetExpr
		{  (fst $4, Sem.make_let $4 Irg.NO_TYPE $6 $3) }
|	LET csim_other OptStar LocatedID COLON Type EQ LetExpr
		{  (fst $4, Sem.make_let $4 $6 $8 $3) }
;
...
ResourceSpec:
	RESOURCE csim_other ResourceList	{ $2 }
;
...
Resource:
	LocatedID
		{ Irg.add_symbol (fst $1) (Irg.RES (fst $1)); $1 }
|	LocatedID LBRACK CARD_CONST RBRACK
		{ Irg.add_symbol (fst $1) (Irg.RES (fst $1)); $1 }
;

CanonSpec:
	CANON csim_other STRING_CONST LPAREN CanonParamList RPAREN
		{ ($3, Irg.CANON_DEF($3, Irg.CANON_FUNC, Irg.NO_TYPE, List.rev $5)) }
|	CANON csim_other Type STRING_CONST LPAREN CanonParamList RPAREN
		{ ($4, Irg.CANON_DEF($4, Irg.CANON_FUNC, $3, List.rev $6)) }
|	CANON csim_other STRING_CONST LPAREN RPAREN
		{ ($3, Irg.CANON_DEF($3, Irg.CANON_FUNC, Irg.NO_TYPE, [])) }
|	CANON csim_other Type STRING_CONST LPAREN RPAREN
		{ ($4, Irg.CANON_DEF($4, Irg.CANON_FUNC, $3, [])) }
|	CANON csim_other Type STRING_CONST
		{ ($4, Irg.CANON_DEF($4, Irg.CANON_CNST, $3, [])) }
;
...
ExceptionSpec:
	EXCEPTION csim_other IdentifierList	{ $3 }
;
...
MemorySpec:
	MEM csim_other LocatedID LBRACK MemPart RBRACK OptionalMemAttrDefList
		{ (fst $3, Sem.make_mem $3 (fst $5) (snd $5) $7) }
;

RegisterSpecHeader: REG LocatedID LBRACK RegPart RBRACK
	{
		Sem.add_spec (fst $2) (Sem.make_reg $2 (fst $4) (snd $4) []);
		Csim.prepare_reg (fst $4) (snd $4);
		$2
	}
...
PortSpecHeader : PORT LocatedID LPAREN PortPart RPAREN  
    {
		Sem.add_spec (fst $2) (Sem.make_port $2 (fst $4) (snd $4) []);
		Csim.prepare_port (fst $4) (snd $4);
		$2
	}
;
...
VarSpec:
	VAR csim_other LocatedID LBRACK RegPart RBRACK OptionalMemAttrDefList
		{ (fst $3, Sem.make_var $3 (fst $5) (snd $5) $7) }
;

MemPart:
	LetExpr COMMA Type
		{ Sem.to_int (Sem.eval_const $1), $3 }
|	LetExpr
		{ Sem.to_int (Sem.eval_const $1), Irg.INT 8 }
;
