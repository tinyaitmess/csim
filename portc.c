 ...
 */
static void  portc_reset(csim_inst_t *inst) {
    portc_inst_t * portc_inst = ( portc_inst_t *)inst;
	portc_inst -> DDRC = 0;
	portc_inst -> PORTC = 0;
 ...
 */
static void portc_construct(csim_inst_t *inst, csim_confs_t confs) {
    portc_inst_t * portc_inst = ( portc_inst_t *)inst;
    portc_reset(inst);
}
 ...
 */
static void portc_destruct(csim_inst_t *inst) {
}
...
static void name_DDRC(csim_inst_t *inst, int num, char *__buffer, int size) {
	portc_inst_t *__inst = (portc_inst_t *)inst;
...
static void on_input_PIN(csim_port_inst_t *port, csim_value_type_t type, csim_value_t val) {
	portc_inst_t *__inst = (portc_inst_t *)port->inst;
...
static int portc_display(char *buf, csim_iocomp_inst_t *inst) {
	char display[] = " ";
	// By default shows nothing, can be modified by changing the display var.
	return sprintf(buf,display);
}

static void portc_on_key(char key, csim_iocomp_inst_t *inst) {
}


csim_iocomp_t  portc_component = {
	{
    "portc",
    CSIM_IO,
    0,					// version
